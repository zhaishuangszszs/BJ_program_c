...
#define DESTMAC3	0x47
#define DESTMAC4	0x70
#define DESTMAC5	0x52

#define destination_ip "192.168.160.134"
...
int main(){
    sock_raw=socket(AF_PACKET,SOCK_RAW,ETH_P_IP);//记得sudo权限才能运行
    if(sock_raw == -1)
...
void get_mac(){
	...
	struct ethhdr *eth = (struct ethhdr *)(sendbuff);
  	...
  	eth->h_dest[3]    =  DESTMAC3;
   	...
   	eth->h_proto = htons(ETH_P_IP);   //0x0800（上层协议IPV4）
...
void get_ip(){
	...
	iph->ihl	= 5;//头部长度20字节
	iph->version	= 4;//IP协议版IPV4
	iph->tos	= 16;//服务类型（Type of Service）最小延迟
	iph->id		= htons(10201);//标识符（Identification）（分包时判断是不是同一包）
	iph->ttl	= 64;// 生存时间（Time to Live）（每过一路由器减一）
	iph->protocol	= 17;// 上层协议类型（Protocol）UDP

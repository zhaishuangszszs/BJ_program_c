...
void ip_header(unsigned char *buffer){
    ...
    printf("\nIP Header\n");
    printf("\t|-Source IP               : %s\n", inet_ntoa(*(struct in_addr*)&ip->saddr));//ip->saddr是unsigned32,需要的参数为struct addr_len
...
void icmp_header(unsigned char *buffer){
     ...
     * Todo("exercise 2 : Complete the code of Step 1 correctly, and submit your source code.")
     */
     struct icmphdr *icmp_hdr = (struct icmphdr*)(buffer + sizeof (struct ethhdr) + sizeof (struct iphdr));
